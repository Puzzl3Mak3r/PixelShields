...
-- local rectUp    = display.newRect( cx, cy, 1900, 600 )
-- local rectDown  = display.newRect( cx, cy, 1900, 600 )
-- local rectLeft  = display.newRect( cx, cy, 1200, 400 )
-- local rectRight = display.newRect( cx, cy, 1200, 400 )

-- local rectUp2   = display.newRect( cx, cy, 200, 100 )
-- -- rectUp.rotation = 45
...
-- Polygons
local upDownPoly        = { 0,0,  1900,0,  1900,600,  1150,1050,  750,1050,  0,600, }
local rectUp            = display.newPolygon( cx, cy - (1050/2), upDownPoly )
local rectDown          = display.newPolygon( cx, cy + (1050/2), upDownPoly )
rectDown.rotation       = 180

local lePoly            = { 0,-30,  800,450,  0,930 }
local riPoly            = { 800,-30,  0,450,  800,930 }
local rectLeft          = display.newPolygon( cx - 600, cy, lePoly )
local rectRight         = display.newPolygon( cx + 600, cy, riPoly )
...
-- Lines for guides
local lineLeft          = display.newLine( 0,0,  cx-200,cy,  0,900 )
local lineRight         = display.newLine( 1900,0,  cx+200,cy,  1900,900 )
lineLeft.strokeWidth    = 20
lineRight.strokeWidth   = 20
lineLeft.alpha          = 0.2
lineRight.alpha         = 0.2
...
local upTextGuide = display.newText( "< UP >", cx, cy-250, native.systemFont, 50 )
local downTextGuide = display.newText( "< DOWN >", cx, cy+250, native.systemFont, 50 )
local leftTextGuide = display.newText( "< LEFT >", cx-500, cy, native.systemFont, 50 )
local rightTextGuide = display.newText( "< RIGHT >", cx+500, cy, native.systemFont, 50 )
...
local function tweenOutGuides()
    transition.to( upTextGuide,     { time=500, delay=900, alpha=0.5, transition=easing.linear} )
    transition.to( downTextGuide,   { time=500, delay=900, alpha=0.5, transition=easing.linear} )
    transition.to( leftTextGuide,   { time=500, delay=900, alpha=0.5, transition=easing.linear} )
    transition.to( rightTextGuide,  { time=500, delay=900, alpha=0.5, transition=easing.linear} )
end
tweenOutGuides()
...
local function specificFlipFunction(self)
    -- 600 x 600 square
    -- 1900 x 900 screen size
    -- X  650 x 1250
    -- Y  150 x 750
    if self.x > 650 and self.x < 1250 and self.y > 200 and self.y < 700 then
        self.enterFrame = nil
        ...
        if self.dir == "up" then
            -- Move left
            transition.to( self, { time = 220, transition=easing.outSine, x=cx+200 } )
            transition.to( self, { time = 220, transition=easing.inSine, y=cy } )
            -- Move down
            transition.to( self, { time = 220, delay = 220,  transition=easing.inSine, x=cx } )
            transition.to( self, { time = 220, delay = 220,  transition=easing.outSine, y=cy+200 } )
        ...
        elseif self.dir == "left" then
            -- Move down
            transition.to( self, { time = 220, transition=easing.inSine, x=cx } )
            transition.to( self, { time = 220, transition=easing.outSine, y=cy+200 } )
            -- Move right
            transition.to( self, { time = 220, delay = 220,  transition=easing.outSine, x=cx+200 } )
            transition.to( self, { time = 220, delay = 220,  transition=easing.inSine, y=cy } )
        ...
        -- Get to center
        transition.to( self, { time=220, delay=220 * 2, transition=easing.linear, x=cx, y=cy } )
    end
end
...
function spawnEnemies()
    if option and playing then
        ...
        local randomSpeed = ((math.random(30, 40))/20) * fasterM
